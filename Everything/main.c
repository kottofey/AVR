 ...
 *  - Прикрутить каким-нибудь образом sleep mode
 *  - Чтение байта из под курсора на LCD
 * ~- Поиск всех устройств 1-wire (команда SEARCH_ROM)
 * ~- Поиск всех устройств 1-wire с алармом (команда ALARM_SEARCH)
 *  - Сделать выключаемую и настраиваемую подсветку LCD (выкл по таймеру)
 *  - идея для сна: повесить клавиатуру на пин с прерыванием, в сон вводить вручную,
 ...
 *  - Допилить функцию DS_SearchROM() дома, когда будет больше одного устройства 1-wire.
